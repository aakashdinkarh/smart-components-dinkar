...
import { Button, Logo, BorderAnimatedContainer } from "smart-components-dinkar";
import { GitHubLogo, getCombinedClass } from "smart-components-dinkar";
...
export function App({ appLoadStartTime }: { appLoadStartTime: number }) {
  ...
  useEffect(() => {
    ...
    mixpanel.track(MIXPANEL_EVENTS.APP_OPENED, {
      [MIXPANEL_EVENT_PROPERTIES.APP_LOAD_TIME]:
        appLoadEndTime - appLoadStartTime,
  ...
  return (
    <main className={styles["app-container"]}>
      <section
        className={getCombinedClass(styles["side-menu"], {
          [styles["show-mobile-nav"]]: mobileSideNavShow,
        })}
      >
        <div
          className={getCombinedClass(
        ...
        <div
          className={getCombinedClass(
            styles["github-logo-container"],
            "border-t width-full-section pt-2"
          )}
        >
          <BorderAnimatedContainer
            borderPositioning="outset"
            animatedBorderColor="#79c0ff"
          >
            <Link
