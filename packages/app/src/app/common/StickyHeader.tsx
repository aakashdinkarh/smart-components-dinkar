...
import { Select, getCombinedClass } from "smart-components-dinkar";
...
function windowResizeEventListenerEffect(
  stickyBelowLogoContainer: RefObject<HTMLDivElement>
) {
...
export function StickyHeader({
  ...
  useEffect(() => {
    ...
    mixpanel.track(MIXPANEL_EVENTS.STICKY_HEADER_RENDER, {
      ...(withThemeSelector && {
        [MIXPANEL_EVENT_PROPERTIES.THEME]: THEME_OPTIONS.find(
          (option) => option.value === codeStyleTheme
        )?.label,
      }),
    });
  ...
  return (
    ...
    >
      <div>
        ...
        {subtitle != null && (
          <p className="mb-1" style={{ marginTop: "-1rem" }}>
            {subtitle}
          </p>
        )}
      </div>
      ...
      {withThemeSelector && (
        <>
          ...
          <link
            rel="stylesheet"
            href={`/codeHighlighterThemes/${codeStyleTheme}.css`}
          ></link>
        </>
      )}
    </div>
