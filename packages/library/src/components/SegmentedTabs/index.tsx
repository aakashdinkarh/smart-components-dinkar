...
export const SegmentedTabs = memo(function SegmentedTabs({
  ...
  const tabs: TabProps[] = validChildren.map(
    (child) =>
      ({
  ...
  const tabsRef: {
    current: Record<string, HTMLDivElement | HTMLButtonElement | null>;
  } = useRef({});
    ...
    <div className="__scd_segmented_tabs_container">
      <div
        className={getCombinedClass(
          styles["overflow-container"],
          "__scd_overflow_container"
        )}
      >
        <div
          className={getCombinedClass(
            styles["tabs-container"],
            "__scd_tabs-container"
          )}
        >
          <div
            ref={(v) => {
              tabsRef.current.slider = v;
            }}
      ...
      <div className="__scd_tab_content">
        {validChildren.filter(
          (child) => (child as { props: TabProps }).props.name === activeTab
        )}
      </div>
    </div>
  );
