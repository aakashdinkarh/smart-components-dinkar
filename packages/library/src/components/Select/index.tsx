import { SelectedOptions } from '@components/Select/SelectedOptions';
...
export const Select = memo(function Select({
	...
	}, [options, multiple, value, selectedValue]);

	const displayValue: string | undefined = getDisplayValue({
		selectedValue,
		options,
		multiple,
	});
	...
	return (
		<div
			ref={containerRef}
			className={getCombinedClass(styles['select-input-container'], className)}
			onKeyDown={onKeyDown}
		>
			<input
				type="text"
			...
			<ul
				ref={listRef}
				className={getCombinedClass(styles['list-options'], {
							...
							<>
								{visibleOptions.map((option, index) => {
									return (
										<li
											key={option.value}
											className={getCombinedClass(styles['list-option'], {
			...
			<div
				onClick={toggleOptionList}
				className={getCombinedClass(styles['caret-button'], {
					[styles.open]: isSelectOpen,
				})}
			/>
		</div>
